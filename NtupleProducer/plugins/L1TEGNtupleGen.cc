...
class L1TEGNtupleGen : public L1TEGNtupleBase {
public:
...
void L1TEGNtupleGen::initialize(TTree &tree, const edm::ParameterSet &conf, edm::ConsumesCollector &&collector) {
  ...
  tree.Branch("simpart_eta", &genpart_eta_);
  tree.Branch("simpart_phi", &genpart_phi_);
  tree.Branch("simpart_pt", &genpart_pt_);
  tree.Branch("simpart_energy", &genpart_energy_);
  tree.Branch("simpart_dvx", &genpart_dvx_);
  tree.Branch("simpart_dvy", &genpart_dvy_);
  tree.Branch("simpart_dvz", &genpart_dvz_);
  tree.Branch("simpart_ovx", &genpart_ovx_);
  tree.Branch("simpart_ovy", &genpart_ovy_);
  tree.Branch("simpart_ovz", &genpart_ovz_);
  tree.Branch("simpart_mother", &genpart_mother_);
  tree.Branch("simpart_exphi", &genpart_exphi_);
  tree.Branch("simpart_exeta", &genpart_exeta_);
  tree.Branch("simpart_exx", &genpart_exx_);
  tree.Branch("simpart_exy", &genpart_exy_);
  tree.Branch("simpart_fbrem", &genpart_fbrem_);
  tree.Branch("simpart_pid", &genpart_pid_);
  tree.Branch("simpart_gen", &genpart_gen_);
  tree.Branch("simpart_reachedEE", &genpart_reachedEE_);
  tree.Branch("simpart_fromBeamPipe", &genpart_fromBeamPipe_);
  tree.Branch("simpart_posx", &genpart_posx_);
  tree.Branch("simpart_posy", &genpart_posy_);
...
void L1TEGNtupleGen::fill(const edm::Event &iEvent, const L1TEGNtupleEventSetup &l1teg_es) {
  ...
  L1TEG_helpers::SimpleTrackPropagator toHGCalPropagator(aField_);
  ...
  for (unsigned int i = 0; i < npart; ++i) {
    ...
    {
      L1TEG_helpers::Coordinates propcoords;
      bool reachesHGCal =
      ...
      L1TEG_helpers::SimpleTrackPropagator indiv_particleProp(aField_);
    ...
    if(reachedEE > 0) {
      L1TEG_helpers::Coordinates hitsHGCal;
